class CalculadoraTarifas {
  ...
  calcularTarifa(ticket) {
    ...
    for (const dia in desglosePorDia) 
    {
      ...
      let costoDia = (diurnas * this.#precioNormalHora) + (nocturnas * this.#precioNocturnoHora);
      
      let detalleDia = `Día ${dia}: ${diurnas} horas diurnas, ${nocturnas} horas nocturnas.`;
      
      if (costoDia > this.#topeDiario) {
        costoDia = this.#topeDiario;
            detalleDia += ` (Tope diario de ${this.#topeDiario} Bs aplicado)`;
        }
        
        if (diurnas > 0 || nocturnas > 0) {
          detalles.push(detalleDia);
        }
        total += costoDia;
    }
    if (horasCobrables > horasExactas) {
      detalles.push(`(Se aplicó redondeo hacia arriba)`);
    }
    ...
    for (const dia in desglosePorDia) 
    {
      ...
      return {
            totalFormateado: `Total: Bs ${totalPerdida}`,
            detalles: `Horas exactas: ${horasExactas.toFixed(2)}. (Se aplicó tarifa por perdida de ticket)`
        };
    }
